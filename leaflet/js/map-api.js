...
(function($) {
	...
	LeafMapApi = function(customOptions) {
		...
		var map;
			...
			// var OpenStreetMap_DE = L.tileLayer('//{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { noWrap: true });
			...
			var options = {
				...
				leafMapOptions: {
					...
					layers: [OpenStreetMap_DE, Esri_WorldImagery],
			...
			function _createMarkers(markers) {
				...
				for (var i = 0; i < markers.length; i++) {
					...
					if (options.usePopup) {
						marker.bindPopup('<div class="marker-content ' + options.leafMarkerIcon.className + ' ">' + options.markerPopupTmpl(markerData) + '</div>', options.popupOptions);
			...
			function createMyMarker(markerData) {
				...
				if (options.usePopup) {
					myMarker.bindPopup('<div class="marker-content ' + options.leafMyMarkerIcon.className + '">' + options.myMarkerPopupTmpl() + '</div>', options.popupOptions);
			...
			var geoLocation = (function () {
				...
				var errorCB = function(error) {
					console.log('geolocation error: %o', error);
					// $('.debugging').html(`ERROR(${error.code}): ${error.message}`);
					...
					$.ajax('https://ipapi.co/json')
						.then(
							...
							function fail(error, status) {
								...
								console.log('error: ' + error.error + ', reason: ' + error.reason);
								latLng = null;
							}
						);
				}
				...
				function init() {
					if (!isInit) {
						if (navigator.geolocation) {
							...
							navigator.geolocation.getCurrentPosition(successCB, errorCB, { timeout: 5000 });
							trackLocID = navigator.geolocation.watchPosition(successCB, errorCB, { timeout: 5000 });
